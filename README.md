...
- **Self-deploy** (`zerops_deploy targetService=appdev`) uses the DEV
  ...
  patterns would destroy the dev workspace on extraction.
...
A minimal modern .NET 9 web project file (`<YourApp>.csproj`):
...
Both property settings are **required** for the standard .NET 9 templates:

- `<ImplicitUsings>enable</ImplicitUsings>` — without it `List<T>`, `Task`,
  `HttpClient`, `IConfiguration` etc. fail to resolve at build with
  `CS0246: The type or namespace name '<X>' could not be found`. Modern
  .NET project templates rely on the implicit-usings feature instead of
  per-file `using` statements.
- `<Nullable>enable</Nullable>` — matches the default analyzer config in
  net9 templates; omitting it will produce a flood of `CS8600`/`CS8602`
  nullability warnings on otherwise-correct code generated against the
  net9 BCL.

`<AssemblyName>` is what the deploy stage runs (`start: dotnet
YourApp.dll`); keep it aligned with the `start:` block in
`zerops.yaml`'s prod setup.
